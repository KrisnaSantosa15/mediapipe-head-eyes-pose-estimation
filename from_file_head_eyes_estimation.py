...
face_mesh = mp_face_mesh.FaceMesh(
    static_image_mode=True, refine_landmarks=True, max_num_faces=1, min_detection_confidence=0.5, min_tracking_confidence=0.5)
...
if results.multi_face_landmarks:
    ...
    cv2.putText(image, f"right eye ratio: {right_eye_ratio:.2f}", (
    ...
    for idx in RIGHT_IRIS:
        cv2.circle(image, mesh_points[idx], 3, (255, 0, 0), -1, cv2.LINE_AA)
        ...
        for idx, lm in enumerate(face_landmarks.landmark):
            if idx == 33 or idx == 263 or idx == 1 or idx == 61 or idx == 291 or idx == 199:
                ...
                cv2.putText(image, str(
                    idx), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1, cv2.LINE_AA)
